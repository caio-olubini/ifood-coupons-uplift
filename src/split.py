...
#: Tipos de oferta que pagam desconto e entram na modelagem. `informational` é
#: ação de comunicação sem desconto associado — fora de escopo (2026-07-10).
MODELED_OFFER_TYPES = ("bogo", "discount")


def exclude_informational(df: DataFrame) -> DataFrame:
    """Remove `offer_type == informational` antes de qualquer ajuste de modelo.

    `informational` não paga desconto (`cost.add_reward_cost`); deixá-lo
    influenciar o baseline preditivo ou o X-learner misturaria um mecanismo de
    recompensa que nenhum modelo do projeto usa. Chamar logo após
    `temporal_split`, antes de `toPandas()`, garante que nenhum modelo —
    baseline, X-learner, avaliação ou curva de ganho — jamais o veja.
    """
    return df.filter(F.col("offer_type").isin(list(MODELED_OFFER_TYPES)))

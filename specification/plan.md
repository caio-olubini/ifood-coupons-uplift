...
| Testes | pytest + `chispa` (ou asserções sobre DataFrame) | Cobre G1–G8 com dados sintéticos minúsculos e determinísticos. |
...
- **`src/contract.py`** — `StructType` do contrato + validação Pydantic de schema e amostra
  (REQ-107).
- **`src/eda.py`** — visões e figuras Plotly (REQ-108) e SMD do balanço (REQ-109).
...
contract.validate(df) -> None                   # levanta se G1–G8 falham (REQ-107)
eda.covariate_balance(df, cfg) -> DataFrame     # SMD por covariável (REQ-109)
```
...
| REQ-103 | `attribution.attribute` |
| REQ-104 | `attribution.build_label` + T-G3, T-G4, T-G5 |
